  ...
  return (
    <div ref={ref} className="h-full flex items-center justify-center">
      <div className="relative w-[220px] h-[220px]">
        {/* Center bot node */}
        <motion.div
          className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-12 h-12 rounded-full bg-accent/15 border border-accent/30 flex items-center justify-center z-10"
        ...
        {inView && activeIdx >= 0 && (
          <motion.div
            ...
            animate={{ width: RADIUS * 2 + 20, height: RADIUS * 2 + 20, opacity: 0 }}
            transition={{ duration: 1.2, ease: "easeOut" }}
          />
        )}

        {/* Connection lines + channel icons */}
            ...
            <g key={channel.label}>
              ...
              <svg className="absolute inset-0 w-full h-full pointer-events-none" style={{ overflow: "visible" }}>
                <motion.line
                  x1="110" y1="110"
                  x2={cx} y2={cy}
                  stroke={isActive ? "hsl(var(--accent)/0.3)" : "hsl(var(--foreground)/0.06)"}
                  strokeWidth="1.5"
                  initial={{ opacity: 0 }}
              ...
              {/* Icon */}
              <motion.div
                className={`absolute w-11 h-11 rounded-full flex items-center justify-center transition-all duration-400 ${
                  isActive
                    ? "bg-accent/15 border border-accent/30 shadow-[0_0_14px_hsl(var(--accent)/0.25)] text-foreground"
